...
const Login = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Resolve User Email and Role via custom profiles database
      ...
      // 3. Coordinate Immediate Role Redirection
      if (authData.user) {
        if (profile.role === "super_admin") {
          navigate("/admin");
        } else if (profile.role === "kiosk_admin") {
      ...
      console.error("Login verification crash:", err.message);
          ...
          <div className="lg:col-span-7 p-8 md:p-16 xl:p-20 flex items-center">
            <div className="max-w-sm w-full mx-auto space-y-10">
              ...
              <p className="text-sm text-slate-500 font-medium text-center">
                New to Credi-Can? <Link to="/signup" className="text-primary font-bold">Create an account</Link>
              </p>
            </div>
          </div>
        </div>
